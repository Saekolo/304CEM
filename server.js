...
http.createServer(function (req,res,err) {
	...
	else if(req.url === "/hmpage"){
		sendFileContent(res, "hmpage.html", "text/html");
	}
        ...
        if (req.method === "POST") {
            ...
            return req.on('data', function (data) {
                ...
                return req.on('end', function () {
                    ...
                    MongoClient.connect(dbUrl, function (err, db)  {
                        ...
                        var dbo = db.db("mydb");
                        var myobj = stringMsg;
                        var query = { name: c[1], email: d[1], message: e[1], id: c[1]};
                        var items = query;
						console.log(query);
							
						dbo.collection("comment").insertOne(query, function(err, res) {
							  if (err) throw err;
         ...
         if (req.method === "POST") {
            ...
            return req.on('data', function (data) {
                ...
                console.log(formData);
                info = formData.split("&");
                console.log(info[0]);//email=XXX

                return req.on('end', function () {
                    var user;
                    user = qs.parse(formData);

                    msg1 = JSON.stringify(user);
                    ...
                    MongoClient.connect(dbUrl, function (err, db)  {
                        ...
                        var dbo = db.db("mydb");
                        var myobj = stringMsg;
                        var query = { email: c[1], id: c[1]};
                        var items = query;
						console.log(query);
							
						dbo.collection("subscriber").insertOne(query, function(err, res) {
							  if (err) throw err;
						 ...
						 res.end("Thank you for your subscription");
					});
				});	
			});
